...
export default {
    ...
    data() {
      return {
          currentIndex: -1, //标识那个下标的分类项需要显示子分类列表
    ...
    computed: {
      ...
      // 一旦使用vuex多模块编程 ...mapState 通过数组中的名称直接读取不行
      //  ...mapState(['ategoryList'])  //categoryList (){return this.$store.state.categoryList}


       ...mapState({
        // 右边是一个回调函数， 回调函数接收是总state,返回值就作为计算属性
    ...
    mounted() {
        // 通过store对象的dispatch()来触发异步 action getCategoryList 执行请求获取数据
        // this.$store.dispatch('getCategoryList')    //数据会重接口中转移到vuex的state 中
