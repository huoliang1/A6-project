...
import {mapState} from 'vuex'
  export default {
    ...
    watch:{
        'tradeInfo.userAddressList'(value){  //地址数据列表有了
              // 从中找出默认地址
             const defaultAddress = value.find(address => address.isDefault==='1')
             this.selectedAddress = defaultAddress || {}  //注册万一没有默认地址就是空
        }
    },
    computed:{
      ...mapState({
      ...
      async submitOrder(){
          ...
          const orderInfo ={
               ...
               consignee,
                ...
                paymentWay: "ONLINE", //支付方式  固定为在线支付
                orderComment,  //用户留言
         ...
         const result = await this.$API.reqSubmitOrder(tradeNo,orderInfo)
            // 如果成功了跳转到支付界面(要携带orderId的query参数)
            if(result.code === 200){
                // 取响应数据的订单id
