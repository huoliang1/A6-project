<template>
  <div class="reginster-conteainer">
    <!-- 注册内容 -->
    <div class="register">
      <h3>注册新用户
      ...
      <div class="content">
        <label>手机号:</label>
        <input name="phone" type="text" placeholder="请输入你的手机号" v-model="mobile"
      ...
      <div class="controls">
        <input type="checkbox" v-model="isAgree" name="协议" v-validate="{agree: true}">
...
<script type="text/ecmascript-6">
  export default {
      ...
      methods: {
          ...
          */
          async register (){
            // 先进行前台表单验证，如果不通过 提示并借宿
            const success = await this.$validator.validateAll() // 对所有表单项进行验证
            // success若果是true 代表效验成功，否则失败
            if(success){
                 // 取出相关数据
